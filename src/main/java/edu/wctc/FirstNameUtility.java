...
public class FirstNameUtility {
    ...
    public int encodeFirstName(String firstName, String middelName) throws MissingNameException {
        int encodedFirstName = 0;
        char firstInitial = firstName.charAt(0);
        char middleInitial = middelName.charAt(0);
        if(firstName.isBlank()){
            throw new MissingNameException("firstName");
        }else{
            if(firstNameCodeMap.containsKey(firstName))
            {
                encodedFirstName += firstNameCodeMap.get(firstName);
            }else{
                encodedFirstName += firstInitialCodeMap.get(String.valueOf(firstInitial));
            }
            if (!middelName.isBlank()) {
                encodedFirstName += middleInitialCodeMap.get(String.valueOf(middleInitial));
            }
